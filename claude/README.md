...
`src/dictionary.md` at the repo root is the master copy. `claudovsky/reference/dictionary.md` here, plus the compact word lists in every platform's `always-on.md`/`per-session.md`, are generated from it by `src/build.py` — edit `src/dictionary.md`, run the script, nothing else needs hand-editing.
